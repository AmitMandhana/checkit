...
const TrackAppointment = () => {
  ...
  const fetchCheckInTime = async () => {
    try {
      const response = await fetch('https://amit19-1.onrender.com/api/doctor-times/latest-check-in');
      if (response.ok) {
  ...
  const fetchNthPatientTime = async () => {
    try {
      const response = await fetch('https://amit19-1.onrender.com/api/prescription/latest');
      if (response.ok) {
        const data = await response.json();
        if (data.createdAt) {
